...
const TAGS = Array.from(new Set(PROJETOS.flatMap((t) => t.tags))).sort();
...
function ProjetosPage() {
  ...
  const filtrados = useMemo(
    () =>
      ativas.length === 0
        ? PROJETOS
        : PROJETOS.filter((t) => ativas.every((a) => t.tags.includes(a))),
    [ativas],
  );
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 pt-32 pb-20">
      <FadeIn>
        <h1 className="text-4xl md:text-5xl font-display font-semibold">Projetos</h1>
            ...
            >
              ...
              >
                <article>
                  {t.capa ? (
                    <div
                      className={`aspect-[1754/1241] overflow-hidden rounded-lg transition-transform duration-500 group-hover:scale-[1.02] ${
                        t.capaContain ? "bg-muted" : ""
                      }`}
                    >
                      <img
                        src={t.capa}
                    ...
                    <div className="image-placeholder aspect-[1754/1241] overflow-hidden transition-transform duration-500 group-hover:scale-[1.02]">
                      [ Imagem do projeto — {t.titulo} ]
                    </div>
                  )}
                  <div className="mt-4">
      ...
      {filtrados.length === 0 && (
        <p className="text-center text-muted-foreground mt-16">
          Nenhum projeto corresponde a essa combinação de etiquetas.
        </p>
      )}
    </section>
  );
